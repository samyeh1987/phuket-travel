...
import { Anchor, CheckCircle, Users, Clock } from 'lucide-react';
...
export default function DivingPage() {
  ...
  const selectedItem = divingTypes.find(d => d.id === selectedType);
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-32 md:pb-0">
      ...
      <div className="max-w-4xl mx-auto px-4 py-6 space-y-4">
        {divingTypes.map(item => (
          <div
            ...
            className={`bg-white rounded-2xl overflow-hidden shadow-sm cursor-pointer transition-all ${selectedType === item.id ? 'ring-2 ring-ocean-500' : 'hover:shadow-md'}`}
          >
            <div className="flex">
            ...
            {/* Expanded Details */}
            {selectedType === item.id && (
              <div className="px-4 pb-4 border-t pt-3 mt-1">
      ...
      {/* Floating CTA */}
      {totalQty > 0 && (
        <div className="fixed bottom-16 md:bottom-0 left-0 right-0 bg-ocean-600 text-white p-4 z-40">
          <div className="max-w-4xl mx-auto flex items-center justify-between">
            <div>
              <div className="text-sm opacity-80">已选 {totalQty} 项</div>
